  ...
  >
    ...
    >
      <li
        ...
        :key="'start-menu-item-' + item.id + '-' + index"
        :title="item.name ?? undefined"
      >
        <div class="start-menu-item p-2 rounded" @click="selectedItem(item)">
          <img
            v-if="item.data.icon"
            :src="item.data.icon"
            class="mx-auto block"
...
import { getItemSearchMap, searchItem } from "../../item/js/index";
...
// 获取开始菜单项目
window.item.getStartMenuItemList();
// 创建滚动条
function createSimpleBar() {
  let element = document.getElementById("start-menu-item-content");
  if (element) {
...
function selectedItem(item: CommonItem) {
  $emit("selected", item, "startMenu");
}
// 设置项目宽度
function setItemWidth() {
  let itemElementList = document.getElementById("start-menu-item-list");
  if (itemElementList) {
...
function mouseout(e: any) {
  ...
  if (getClassElement(e, "start-menu-item")) {
    ...
    removeStyle(e, "start-menu-item", style);
  }
}
// 监听
let onGetStartMenuItemListListen: Function | null = null;
// moutned
onMounted(() => {
  // 刷新DOM完毕执行
  nextTick(() => {
  ...
  // 监听获取开始菜单项目
  onGetStartMenuItemListListen = window.item.onGetStartMenuItemList((data) => {
    // 数据
    itemList = data;
    // 搜索内容清空
...
onUnmounted(() => {
  // 删除监听
  if (onGetStartMenuItemListListen) {
    onGetStartMenuItemListListen();
  }
});
</script>
../js/item ../../../../types/item
