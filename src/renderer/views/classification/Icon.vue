<template>
  ...
  >
    <div class="overflow-hidden h-[inherit]">
      <TopTitleBar :title="$store.state.currentLanguage.setClassificationIcon" v-model:show="s"></TopTitleBar>
      <div class="text-[20px] w-full content">
        <div
          id="classification-icon-list"
          class="flex flex-wrap mx-[6px] pb-[4px] border-b-[1px]"
          :style="{ borderColor: $store.state.setting.appearance.theme.border }"
        >
          <span
            class="w-[36px] h-[36px] flex items-center justify-center mx-[2px] rounded classification-icon"
            :style="{ backgroundColor: selected == key ? $store.state.setting.appearance.theme.minorBackground : null }"
            :title="key"
            v-for="(values, key) in emoji"
            :key="key"
            @mouseover="$styleMouseover($event, 'classification-icon', ['background-color'], [$store.state.setting.appearance.theme.minorBackground])"
            @mouseout="$styleMouseout($event, 'classification-icon', selected == key ? [] : ['background-color'])"
            @click="selected = key"
            >{{ values[0]["value"] }}
          </span>
        </div>
        <div id="classification-icon-item-list" data-simplebar :key="selected">
          <ul class="flex flex-wrap my-[4px] mx-[6px]">
            <li
              class="w-[36px] h-[36px] flex items-center justify-center mx-[2px] rounded truncate classification-icon-item"
              v-for="emoji in emoji[selected]"
              :key="emoji"
              :title="emoji.name"
              @mouseover="$styleMouseover($event, 'classification-icon-item', ['background-color'], [$store.state.setting.appearance.theme.minorBackground])"
              @mouseout="$styleMouseout($event, 'classification-icon-item', ['background-color'])"
              @click="setIcon(emoji.value)"
            >
              {{ emoji.value }}
            </li>
          </ul>
...
export default {
  ...
  methods: {
    ...
    resize() {
      document.getElementById("classification-icon-item-list").style.height =
        document.getElementById("classification-icon").offsetHeight - document.getElementById("classification-icon-list").offsetHeight - 34 + "px";
    },
    setIcon(icon) {
      this.$emit("setIcon", this.id, this.parentId, icon);
    },
  },
};
</script>
